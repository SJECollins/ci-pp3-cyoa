...
def front_door():
    ...
    slowprint("You race for the front door, hoping to escape from the house. "
              ...
              f"'{username},' whispers the shadowy figure as it approaches.\n"
              "You fumble with the door and, too late, realise the bolt at "
              "the top is locked,\nout of reach.\n")
    if "teddy" in pick_ups:
        slowprint("In a panic, you throw Burt at the shadowy figure. It "
                  "catches him. As it seems\nto stare at Burt in confusion, "
    ...
    if window_closed:
        ...
        if "golf_club" in pick_ups:
            slowprint("You run over to the window and try to slide it open, "
                      ...
                      f"'{username},' the shadow softly growls.\n"
                      "You swing the club..\n")
            time.sleep(1)
            escape_house()
        else:
...
def wake_win():
    ...
    slowprint(f"'{username}, why is there milk all over the kitchen floor?'\n"
              "Your eyes open. Light seeps in around your blankets. You throw "
              "them back.\n"
...
def play_again():
    ...
    """
    time.sleep(2)
    user_choice = get_user_choice(f"Well, {username}, would you like to try "
                                  "again?\n"
